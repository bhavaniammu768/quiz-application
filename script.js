const allQuestions = [
 ...
 {q:"Which is a Python IDE?", o:["VS Code","Word","Paint","Excel"], a:0},
...
function loadSet() {
    currentSet = allQuestions.slice(startIndex, startIndex + 15);
    index = 0;
    score = 0;
    loadQuestion();
}
...
function loadQuestion() {
    ...
    document.getElementById("qno").innerText = `Question ${index + 1} of 15`;
...
function checkAnswer(btn, i) {
    clearInterval(timer);
    let correct = currentSet[index].a;
...
function restartQuiz() {
    startIndex += 15;
    if (startIndex >= allQuestions.length) startIndex = 0;
